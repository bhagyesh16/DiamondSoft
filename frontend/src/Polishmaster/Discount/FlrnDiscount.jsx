...
const FlrnDiscount = () => {
    ...
    const fetchPricelistData = async () => {
        try {
            const response = await axios.get('https://diamondsoft-backend.onrender.com/api/fdiscount');
    ...
    const handleEdit = async (id) => {
        try {
            const response = await axios.get(`https://diamondsoft-backend.onrender.com/api/fdiscount/${id}`);
            ...
            setFlrnData(pricelistData.flrn);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await axios.put(`https://diamondsoft-backend.onrender.com/api/fdiscount/${formData.id}`, UpdatedFormData);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            await axios.delete(`https://diamondsoft-backend.onrender.com/api/fdiscount/${id}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const InsertedFormData = {
                ...
                shape: selectedshape,
                purity: selectedPurity,
                flrn: selectedFlrn === "" ? null : selectedFlrn,
            ...
            await axios.post('https://diamondsoft-backend.onrender.com/api/fdiscount', InsertedFormData);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            await axios.post('https://diamondsoft-backend.onrender.com/api/fdiscount/upload', formData, {
                headers: {
        ...
        <>
            <div className="grid grid-cols-1 md:grid-cols-8 p-2">
                ...
                <div className="col-span-1 md:col-span-2 p-2">
                    ...
                    <form>
                        <div className="bg-teal-200 p-4 rounded-lg">
                            <h1 className="text-2xl font-bold mb-4">SET FLRN DISCOUNT</h1>
                            <label className="block">ENTER FLRN DISCOUNT ID</label>
                            <input
                                type="number"
                                name="id"
                                value={formData.id}
                                onChange={(e) => setFormData({ ...formData, id: e.target.value })}
                                className="mt-1 p-2 block w-full rounded-md border border-gray-300 shadow-sm focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50"
                            />
                            ...
                            <input
                                ...
                                className="mt-1 p-2 block w-full rounded-md border border-gray-300 shadow-sm focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50"
                            />
                            <label className="block mt-4">SELECTED FLRN</label>
                            <FlrnDropdown value={selectedFlrn} onChange={(value) => setFlrnData(value)} disabled={formData.price_type === 'own'} />
                ...
                <div className="col-span-1 md:col-span-6 p-2">
                    ...
                    <div className="bg-teal-200 w-full p-2 mb-4 rounded-lg flex justify-center items-center">
                        <label className="text-lg font-bold">UPLOAD YOUR FLRN DISCOUNT CSV</label>
                        <input
                            type="file"
                            accept=".csv"
                    ...
                    <div className="bg-teal-200 w-full p-4 rounded-lg">
                        <h2 className="text-xl font-bold mb-4">FLRN DISCOUNT Data</h2>
                        <table className="colour-table w-full text-center border-collapse border-2 border-black">
                            <thead>
                                <tr>
                                    ...
                                    <th className="px-4 py-2 border-2 border-black">Flrn</th>
                            ...
                            <tbody>
                                {pricelistData.map((item, index) => (
                                    <tr key={item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-teal-100'}`}>
                                        ...
                                        <td className="px-4 py-2 border-2 border-black">{item.flrn_name}</td>
