...
import MilkyDropdown from '../MilkyDropdown';
...
const MilkyDiscount = () => {
    ...
    const fetchPricelistData = async () => {
        try {
            const response = await axios.get('https://diamondsoft-backend.onrender.com/api/mdiscount');
    ...
    const handleEdit = async (id) => {
        try {
            const response = await axios.get(`https://diamondsoft-backend.onrender.com/api/mdiscount/${id}`);
            ...
            setSelectedMilky(pricelistData.milky);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await axios.put(`https://diamondsoft-backend.onrender.com/api/mdiscount/${formData.id}`, UpdatedFormData);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            await axios.delete(`https://diamondsoft-backend.onrender.com/api/mdiscount/${id}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const InsertedFormData = {
                ...
                shape: selectedshape,
                purity: selectedPurity,
                milky: selectedmilky === "" ? null : selectedmilky,
            ...
            await axios.post('https://diamondsoft-backend.onrender.com/api/mdiscount', InsertedFormData);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            await axios.post('https://diamondsoft-backend.onrender.com/api/mdiscount/upload', formData, {
                headers: {
            ...
            <div className='grid grid-cols-1 md:grid-cols-8 p-2'>
                ...
                <div className='col-span-1 md:col-span-2 p-2'>
                    <form className='bg-teal-200 p-4 rounded-lg'>
                        <div className="">
                            <h1 className="text-2xl font-bold mb-4">SET MILKY DISCOUNT</h1>
                            <label className="block">ENTER MILKY DISCOUNT ID</label>
                            <input
                                type="number"
                                name="id"
                                value={formData.id}
                            ...
                            <input
                                ...
                                className="mt-1 p-2 block w-full rounded-md text-black border-gray-300 shadow-sm focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50"
                            />
                            <label className="block mt-4">SELECTED MILKY</label>
                            <MilkyDropdown value={selectedmilky} onChange={(value) => setSelectedMilky(value)} />
                ...
                <div className='col-span-1 md:col-span-6 p-2'>
                    ...
                    <div className='bg-teal-200 w-full mb-4 p-2 sm:flex justify-center items-center rounded-lg'>
                        <label className="text-lg font-bold ">UPLOAD YOUR MILKY DISCOUNT CSV</label>
                        <input
                            type="file"
                            accept=".csv, .xlsx"
                    ...
                    <div className="bg-teal-200 w-full p-4 rounded-lg">
                        <h2 className="text-xl font-bold mb-2">Milky DISCOUNT DATA</h2>
                        <table className="colour-table w-full text-center border-collapse border-2 border-black">
                            <thead>
                                <tr>
                                    ...
                                    <th className="px-4 py-2 border-2 border-black">Milky</th>
                            ...
                            <tbody>
                                {pricelistData.map((item, index) => (
                                    <tr key={item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-teal-100'}`}>
                                        ...
                                        <td className="px-4 py-2 border-2 border-black">{item.milky_name}</td>
